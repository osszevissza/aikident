---
title: Rólunk
description: >-
  Ismerkedjen meg Dr. Kelemen Lászlóval és az Aiki Dent Fogászat küldetésével:
  holisztikus, biológiai és integrált fogászat Budapesten — alapértékeink,
  elkötelezettségünk és az egész embert támogató ellátás.
blocks:
  # ---------------------------------------------------------------- hero ---
  - type: pageHero
    eyebrow: Rólunk
    title: A holisztikus és biológiai fogászat képezi a rendelőnk alapját
    lead: >-
      A high-tech és a többezeréves kínai orvosi diagnosztika találkozása — olyan
      empatikus, biológiai alapú fogászati ellátás, amely az embert egészében
      támogatja.
    actions:
      - label: Időpontfoglalás
        url: /kapcsolat/
        icon: calendar
      - label: Szolgáltatásaink
        url: /szolgaltatasok/
        icon: arrow-right
        style: btn--ghost
    pills:
      - text: Higanymentes fogászat
        icon: shield
      - text: BPA-mentes tömések
        icon: droplet
      - text: Hagyományos kínai orvosi diploma
        icon: needle
      - text: Zeiss Extaro 300 mikroszkóp
        icon: microscope

  # -------------------------------------------------------------- letter ---
  - type: letter
    bg: cream
    eyebrow: Köszöntő
    title: Kedves Páciensek és Barátok!
    lead: >-
      A holisztikus és biológiai fogászat képezi a rendelőnk alapját — Dr. Kelemen
      László a fogorvos és tulajdonos.
    body:
      - >-
        Eszközparkunk az új felfogású, preventív — megelőző — szemléletű fogorvosi
        terápiás stratégiát segíti, a rendelő filozófiája pedig az egész ember
        támogatására épül.
    quote: A high-tech és a többezeréves kínai orvosi diagnosztika találkozása.
    paragraphs:
      - >-
        Szeretettel köszöntöm Önt az Aiki Dent Fogászatban. Melegszívűségem,
        lelkesedésem és gondoskodó természetem tökéletesen illeszkedik ahhoz a
        filozófiához, amelyet Ön már megismert és amelyben megbízik. Bízom benne,
        hogy barátságosnak, együttérzőnek és elkötelezettnek talál mind a fogászati
        egészsége, mind az általános jóléte iránt.
      - >-
        Az integrált orvoslás szemléletének és módszereinek tanulmányozásával
        foglalkozom, és folyamatosan képzem magam — hogy Önnek a lehető
        legszélesebb rálátást nyújthassam az egészségére.
    sign:
      photo: /img/demo1.webp
      name: Dr. Kelemen László
      role: fogorvos, alapító és tulajdonos

  # -------------------------------------------------------- qualifications ---
  - type: split
    bg: paper
    image: /img/demo1.webp
    imageAlt: Dr. Kelemen László, az Aiki Dent Fogászat alapítója
    eyebrow: Bemutatkozás
    title: Dr. Kelemen László
    lead: >-
      A holisztikus és biológiai fogászat képezi a rendelőnk alapját. Dr. Kelemen
      László a fogorvos és tulajdonos.
    body:
      - >-
        A Semmelweis Egyetem (SOTE) Fogorvostudományi Karán szerzett diplomát, majd
        a magyarországi biológiai fogászat vezető szakértőitől tanult tovább, többek
        között Dr. Csiszár Róberttől. Az amerikai IAOMT és az új MOK tagja.
      - >-
        A fogászat mellett a hagyományos kínai orvoslás és a természetgyógyászat is
        a mindennapi munkájának része: a Dr. Eőry Ajándok által vezetett budapesti
        Kínai Klinikán hagyományos kínai orvosi diplomát kapott, mesterei kínai
        professzorok voltak, valamint a világhírű Pálos István professzor.
    checks:
      - icon: award
        title: Fogorvosi diploma
        text: A Semmelweis Egyetem (SOTE) Fogorvostudományi Karán szerzett diplomát.
      - icon: leaf
        title: Biológiai fogászat
        text: >-
          A magyarországi biológiai fogászat vezető szakértőitől tanult, többek
          között Dr. Csiszár Róberttől.
      - icon: users
        title: Szakmai tagságok
        text: >-
          Az amerikai IAOMT (International Academy of Oral Medicine and Toxicology),
          valamint az új MOK (vezető: dr. Tamasi József) tagja.
      - icon: needle
        title: Hagyományos kínai orvosi diploma
        text: >-
          Dr. Eőry Ajándok által vezetett budapesti Kínai Klinikán hagyományos kínai
          orvosi diplomát kapott. Mesterei kínai professzorok voltak, valamint a
          világhírű Pálos István professzor.
      - icon: hand-heart
        title: Természetgyógyászat és akupunktúra
        text: >-
          Természetgyógyászati végzettsége is van. Akupunktúrás szakvizsgát is tett a
          HIETE-n (Haynal Imre Egészségtudományi Egyetem).
      - icon: zap
        title: Lézerterápia és fülakupunktúra
        text: >-
          Lézerterapeuta, valamint a fülakupunktőri szakképzettséggel is rendelkezik.

  # -------------------------------------------------------------- values ---
  - type: cards
    bg: cream
    align: center
    eyebrow: Alapértékeink
    title: Munkánkat alapvető értékeink vezérlik
    lead: >-
      Integritás minden cselekedetben, jelenlét és együttérzés, kíváncsiság és
      fejlődés, felelősségvállalás, valamint az egyéniség helyett az együttműködés.
    cols: 3
    style: mini
    data: values

  # ------------------------------------------------------------- mission ---
  - type: split
    bg: paper
    flip: true
    image: /img/demo2.webp
    imageAlt: Biológiai és holisztikus kezelés az Aiki Dent Fogászatban
    eyebrow: Küldetésünk
    title: Biológiai kezelések és szolgáltatások az Aiki Dent Fogászatban
    lead: >-
      Az Aiki Dent Fogászatban minden tevékenységünk a küldetésünkön alapul: olyan
      empatikus, biológiai alapú fogászati ellátást nyújtani, amely az embert
      egészében támogatja.
    body:
      - >-
        Hiszünk abban, hogy a szájápolás szorosan összefügg az általános egészségi
        állapottal, és átgondolt, integrált fogászati kezelésünk révén segítünk
        pácienseinknek abban, hogy magabiztosságot, önbizalmat és egész életen át
        tartó vitalitást nyerjenek.
    bullets:
      - Empatikus, biológiai alapú ellátás, amely az embert egészében támogatja
      - Integrált kezelési tervek, amelyek a szájüreg és a teljes szervezet összefüggéseire épülnek
      - Biokompatibilis anyagok és minimálisan invazív technikák a természetes fogszerkezet megőrzéséért

  # --------------------------------------------------------------- stats ---
  - type: stats
    bg: wine
    data: stats

  # ---------------------------------------------------------- commitment ---
  - type: split
    bg: cream
    image: /img/demo3.webp
    imageAlt: Gondoskodó fogászati ellátás az Aiki Dent Fogászatban
    eyebrow: Elkötelezettségünk
    title: Nem csupán fogakat kezelünk
    lead: >-
      Nem csupán fogakat kezelünk — hanem segítünk az embereknek abban, hogy
      egészségesebb, teljesebb életet éljenek.
    body:
      - >-
        Az integritás, az együttérzés, a kíváncsiság, a felelősségvállalás és az
        együttműködés alapértékeink által vezérelve olyan ellátást nyújtunk, amely
        átgondolt, tiszteletteljes és valóban a páciensre összpontosít.
    actions:
      - label: Szolgáltatásaink
        url: /szolgaltatasok/
        icon: arrow-right

  # ------------------------------------------------------- testimonials ---
  - type: quotes
    bg: paper
    eyebrow: Páciensvélemények
    title: Amit pácienseink mondanak
    lead: >-
      Néhány visszajelzés azoktól, akik már megjárták velünk az utat a holisztikus
      szemléletű fogászathoz.
    data: testimonials
    footnote: >-
      A vélemények bemutató (lorem ipsum) tartalmak — éles indulás előtt
      cserélendők valódi páciensvéleményekre.
---
