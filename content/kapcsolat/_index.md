---
title: Kapcsolat
description: >-
  Vegye fel velünk a kapcsolatot: Aiki Dent Fogászat, 1136 Budapest, Hegedűs Gyula
  utca 29/b. Telefon: +36 30 193 2714 — időpontot telefonon, e-mailben vagy az
  e-mailben tud kérni.
blocks:
  # ---------------------------------------------------------------- hero ---
  - type: pageHero
    eyebrow: Kapcsolat
    title: Időpontkérés és elérhetőségeink
    lead: >-
      Rendelőnk előzetes egyeztetés alapján működik. Időpontot telefonon vagy
      e-mailben tud kérni.
    actions:
      - label: '+36 30 193 2714'
        url: /kapcsolat/#elerhetoseg
        icon: phone
    pills:
      - text: '1136 Budapest, Hegedűs Gyula utca 29/b'
        icon: pin
      - text: '+36 30 193 2714'
        icon: phone
      - text: 'Nyitvatartás: előzetes egyeztetés alapján'
        icon: clock

  # ------------------------------------------------------------- contact ---
  - type: contact
    id: elerhetoseg
    bg: cream
    eyebrow: Kapcsolat
    title: Így érhet el minket
    lead: >-
      Kérdése van, vagy időpontot szeretne kérni? Hívjon minket telefonon, vagy írjon
      e-mailt.
    # Nincs űrlap: a rendelő nem gyűjt személyes adatot a honlapon keresztül
    # (ez adatkezelési tájékoztatót, megőrzési szabályt és karbantartást vonna maga
    # után). A telefon, az e-mail és a térkép a kapcsolatfelvétel útja.
    showMap: true

  # --------------------------------------------------------- getting here ---
  - type: split
    bg: paper
    flip: true
    image: /img/demo4.webp
    imageAlt: Az Aiki Dent Fogászat rendelője a Hegedűs Gyula utcában
    eyebrow: Megközelítés
    title: Így talál meg minket
    lead: >-
      Rendelőnk a XIII. kerületben, a Hegedűs Gyula utca 29/b szám alatt várja Önt,
      előzetes egyeztetés alapján.
    body:
      - >-
        Kérjük, látogatását minden esetben egyeztesse előre telefonon vagy e-mailben,
        hiszen nyitvatartásunk: előzetes egyeztetés alapján. Így biztosíthatjuk, hogy
        kellő időt és figyelmet szentelhessünk Önnek.
      - >-
        Fontos tudnivaló a fizetésről: fogászati biztosítást nem fogadunk el fizetési
        módként, a díjakat a szolgáltatások igénybevételekor közvetlenül a páciens
        fizeti meg rendelőnkben. Szolgáltatásként számlát állítunk ki a fogászati
        biztosítóknak, így Ön közvetlenül a saját fogászati biztosítójától kaphatja
        meg a visszatérítést — mértéke a konkrét biztosítási csomagtól függ.
    bullets:
      - A pontos útvonalat a fenti térkép és a Google Maps útvonalterv mutatja
      - >-
        Ha elakadna az útvonalon vagy a bejáratnál, hívjon minket, és segítünk
        megtalálni a rendelőt
      - >-
        Ha segítségre van szüksége a megközelítéshez vagy a rendelőn belül, jelezze
        előre telefonon — megnézzük, hogyan tudjuk megkönnyíteni

  # ----------------------------------------------------------------- faq ---
  - type: faq
    bg: cream
    eyebrow: Gyakori kérdések
    title: Kapcsolatfelvétel előtt
    lead: >-
      A leggyakoribb gyakorlati kérdések: időpont, fizetés, ár és az első vizit.
    data: faq
    limit: 4
    actions:
      - label: Összes kérdés
        url: /uj-paciens/#gyik
        icon: arrow-right
---
